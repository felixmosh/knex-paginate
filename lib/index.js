...
module.exports.attachPaginate = function attachPaginate() {
  function paginate({ perPage = 10, currentPage = 1, isFromStart = false, isLengthAware = false }) {
    ...
    const shouldSkipPagination = perPage === 0;

    if (currentPage < 1 || shouldSkipPagination) {
      currentPage = 1;
    }
    //perPage === 0 get all without pagination
    const shouldFetchTotals =
      (isLengthAware || currentPage === 1 || isFromStart) && !shouldSkipPagination;
    let pagination = {};
    let countQuery = null;
    ...
    // This will paginate the data itself, unless perPage is true
